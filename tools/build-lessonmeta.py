...
"module-0/lesson-01.html": {"p":"None — the very first lesson, from absolute zero.","m":[{"w":"A computer 'understands' your program.","r":"It only executes a tiny menu of trivial steps (load, add, jump, store) billions of times a second; the intelligence is an illusion built from speed and quantity."},{"w":"Higher clock speed (GHz) means a faster computer.","r":"Not across designs — a core often spends most of its time waiting on memory, not computing, so GHz alone predicts little."}]},
"module-0/lesson-02.html": {"p":"Lesson 0.1.","m":[{"w":"A CPU spends most of its silicon on arithmetic.","r":"Most of it is caches and control logic to avoid waiting; arithmetic is a small slice. That is the price of low latency."},{"w":"Caches make all memory access fast.","r":"They only speed up predictable reuse; streaming once through data larger than the cache is still bandwidth-bound at full memory latency."}]},
"module-0/lesson-03.html": {"p":"Lessons 0.1–0.2.","m":[{"w":"A GPU is just a faster CPU.","r":"Each GPU core is slower per task; it wins only through thousands of cores and by hiding memory stalls. Hand it serial work and a laptop beats it."},{"w":"Having more threads than cores is wasteful.","r":"Over-subscription is the fuel of latency hiding: while one warp waits on memory another runs, so a GPU wants tens of thousands of threads in flight."}]},
"module-0/lesson-04.html": {"p":"Lessons 0.1–0.3.","m":[{"w":"A 100×-faster kernel makes the whole program 100× faster.","r":"Amdahl's law: a 10% serial part caps the speedup at 10× no matter how fast the kernel, and copy overhead cuts it further."},{"w":"The GPU runs the program and the CPU assists.","r":"It is the reverse — the CPU (host) runs the program and dispatches parallel jobs and data to the GPU (device) across a comparatively slow bridge."}]},
"module-0/lesson-05.html": {"p":"Lessons 0.1–0.4.","m":[{"w":"GPUs were designed for AI.","r":"They were built for graphics (GeForce 256, 1999); their fitness for AI was discovered — crystallized by AlexNet in 2012 — not planned."},{"w":"Each new GPU generation is a fundamentally new machine.","r":"From GeForce 256 to Rubin the fundamentals are unchanged (thousands of simple cores hiding latency with parallelism); only scale and specialization grew."}]},
"module-0/lesson-06.html": {"p":"Lessons 0.1–0.5.","m":[{"w":"CUDA is how all GPUs work.","r":"CUDA is NVIDIA-only; AMD (ROCm/HIP), Intel (oneAPI/SYCL), and portable standards (WebGPU, OpenCL) exist, and the concepts transfer across all of them."},{"w":"You need an expensive GPU to learn this.","r":"Free Colab, in-browser WebGPU labs, and cheap cloud rentals cover the whole course; the barrier is knowledge, not hardware."}]},
